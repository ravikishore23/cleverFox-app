...
import { TaskModel, type TaskStatus } from "../models/Task.js";
...
function recomputeFromSubTasks(task: any) {
...
tasksRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const subTasks = Array.isArray(subTasksRaw)
      ? subTasksRaw
          .map((t: any) => String(t?.title ?? t ?? "").trim())
          .filter(Boolean)
          .slice(0, 100)
          .map((t: string) => ({
            title: t,
    ...
    const created = await TaskModel.create({ title, dueAt, subTasks });
...
tasksRouter.patch(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (subTaskAddRaw !== undefined) {
      ...
      task.subTasks.push({ title, done: false, createdAt: new Date() } as any);
    }
    ...
    if (subTaskToggleRaw !== undefined) {
      ...
      const sub = task.subTasks.find((st: any) => String(st._id) === subId);
      if (!sub)
    ...
    if ((task.subTasks?.length ?? 0) === 0) {
      if (statusRaw !== undefined) {
        const status = String(statusRaw) as TaskStatus;
        if (
          !(
            status === "pending" ||
            status === "inProgress" ||
