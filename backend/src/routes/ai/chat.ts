...
import { type AiMessage } from "../../ai/types/chat.js";
...
aiRouter.post("/ai/chat", async (req, res, next) => {
  try {
    ...
    const providerMessages = [
      ...([
        typeof buildStudyRoomSystemPrompt() === "string"
          ...
          : {
              ...(buildStudyRoomSystemPrompt() as AiMessage),
              content:
                (buildStudyRoomSystemPrompt() as AiMessage).content +
                searchContext,
            },
      ] as AiMessage[]),
      ...body.messages.map((m) => ({
    ...
    const response: ChatResponse & { chatId?: string } = {
      ok: true,
...
aiRouter.post("/ai/agent-chat", async (req, res) => {
  try {
    const body = req.body as ChatRequest & { chatId?: string };
    ...
    const autoActions = proposal.actions.filter((a) =>
      safeAutoTools.has(a.tool),
    );
    const pendingActions = proposal.actions.filter(
