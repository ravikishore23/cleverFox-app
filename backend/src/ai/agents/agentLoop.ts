...
import type { FolderAnalysis } from "../localAgent.js";
...
export type ProposedAction = {
  ...
  fileFormat?: "pdf" | "txt" | "md" | "html" | "json" | "csv";
  fileName?: string;
  fileContent?: string;
  outputDir?: string;
};
...
function detectRequestedFormat(
  text: string,
): "pdf" | "txt" | "md" | "html" | "json" | "csv" | null {
  const lower = text.toLowerCase();

  // If the user seems to be negating the intent, bail out of the heuristic early
  if (
    /\b(?:don'?t|do not|no|stop|cancel|instead of)\b.*?\b(?:want|need|make|create|generate|pdf|csv|json|html|md|txt|text)\b/i.test(
      lower,
    )
  ) {
    return null;
  ...
  if (
    /(\bmake\b|\bcreate\b|\bgenerate\b|\bexport\b|\bsave\b).*?(\bpdf\b|\.pdf\b)/i.test(
      lower,
    )
  )
    return "pdf";
  if (
    /(\bmake\b|\bcreate\b|\bgenerate\b|\bexport\b|\bsave\b).*?(\bcsv\b|\.csv\b)/i.test(
      lower,
    )
  )
    return "csv";
  if (
    /(\bmake\b|\bcreate\b|\bgenerate\b|\bexport\b|\bsave\b).*?(\bjson\b|\.json\b)/i.test(
      lower,
    )
  )
    return "json";
  if (
    /(\bmake\b|\bcreate\b|\bgenerate\b|\bexport\b|\bsave\b).*?(\bhtml\b|\.html\b)/i.test(
      lower,
    )
  )
    return "html";
  if (
    /(\bmake\b|\bcreate\b|\bgenerate\b|\bexport\b|\bsave\b).*?(\bmarkdown\b|\bmd\b|\.md\b)/i.test(
      lower,
    )
  )
    return "md";
  if (
    /(\bmake\b|\bcreate\b|\bgenerate\b|\bexport\b|\bsave\b).*?(\btext\b|\btxt\b|\.txt\b)/i.test(
      lower,
    )
  )
    return "txt";
...
const CODE_RUNNERS: Record<
  string,
  { cmd: string; buildArgs: (f: string) => string[] }
> = {
...
/* ------------------------------------------------------------------ */
/*  Intent Detection (comprehensive regex-based)                       */
/* ------------------------------------------------------------------ */
...
function detectIntent(text: string): DetectedIntent {
  ...
  const m1 = lower.match(
    new RegExp(
      `(?:write|create|generate|make|build|develop)\\s+(?:a\\s+)?(?:some\\s+)?(?:the\\s+)?(${LANGS_PATTERN})\\s+(?:code|script|program|function|class|module|app|application|file|project)\\s+(?:for|to|that|which)\\s+(.+?)$`,
      "i",
    ),
  );
  if (m1) {
  ...
  if (!writeCode) {
    const m2 = lower.match(
      /(?:write|create|generate|make|build|develop)\s+(?:a\s+)?(?:some\s+)?(?:the\s+)?(?:code|script|program|function|app)\s+(?:for|to|that|which)\s+(.+?)$/i,
    );
    if (m2) {
      writeCode = true;
      codeLanguage = "python";
  ...
  if (!writeCode) {
    const m3 = lower.match(
      /(?:write|create|generate|make|build)\s+(?:a\s+)?(?:some\s+)?(function|class|module|api|server|website|webpage|component|library|tool|utility|helper)\s+(?:for|to|that|which|called|named)\s+(.+?)$/i,
    );
    if (m3) {
      writeCode = true;
      codeLanguage = "python";
  ...
  if (!writeCode) {
    const m5 = lower.match(
      new RegExp(
        `^(${LANGS_PATTERN})\\s+(?:code|script|program)\\s+(?:for|to|that)\\s+(.+?)$`,
        "i",
      ),
    );
    if (m5) {
  ...
  if (!writeCode) {
    const m6 = lower.match(
      /(?:open|launch)\s+(?:vscode|vs\s+code|code)\s+(?:and\s+)?(?:write|create|generate)\s+(?:a\s+)?(?:some\s+)?(.+?)$/i,
    );
    if (m6) {
      writeCode = true;
      const rest = m6[1];
  ...
  if (!writeCode) {
    const m7 = lower.match(
      /(?:help\s+me\s+|can\s+you\s+|please\s+)?(?:code|program|develop)\s+(?:a\s+|an\s+|the\s+)?(.+?)$/i,
    );
    if (m7 && m7[1].length > 5) {
      writeCode = true;
      codeLanguage = "python";
  ...
  /* ── Obsidian / file save ──────────────────────────────────── */
  const saveToFile =
    /(?:save|put|write|store|create)\s+(?:it\s+)?(?:to|into|in|on)\s+(?:obsidian|file|note)/i.test(
      lower,
    ) ||
  ...
  /* ── Read file ─────────────────────────────────────────────── */
  const readFile =
    /(?:read|show|display|cat)\s+(?:the\s+)?(?:file|content)/i.test(lower);

  /* ── List directory ────────────────────────────────────────── */
  const listDir =
    /(?:list|show|display|ls|dir)\s+(?:the\s+)?(?:files|directory|folder|dir)/i.test(
      lower,
    );

  /* ── Run command ───────────────────────────────────────────── */
  const runCommand =
    /(?:run|execute)\s+(?:the\s+)?(?:command|cmd)/i.test(lower) ||
  ...
  const searchMatch = lower.match(
    /(?:search|google|find|look\s+up|look\s+for)\s+(?:for\s+)?(.+?)\s+(?:in|on|using)\s+(chrome|brave|firefox|edge)/i,
  );
  if (searchMatch) {
    browserSearch = true;
  ...
  if (!browserSearch) {
    const simple = lower.match(
      /^(?:search|google|find|look\s+up)\s+(?:for\s+)?(.+?)$/i,
    );
  ...
  if (!browserSearch && !writeCode) {
    const openMatch = lower.match(
      /(?:open|launch|start|run)\s+(?:the\s+)?(?:app\s+|application\s+)?(.+)$/i,
    );
  ...
  const pathMatch = text.match(
    /(?:in|to|at|from|path)\s+[`"']?([A-Za-z]:\\[^\s`"']+|\/[^\s`"']+)[`"']?/i,
  );
  ...
  if (!targetPath && process.env.USERPROFILE) {
    ...
    const wellKnownPaths: Record<string, string> = {
...
export async function proposeActions(
  ...
  const wantsListNotes =
    /\b(list|show|display|available|all)\b.*\bnotes?\b/i.test(userText) ||
    /\bnotes?\b.*\b(list|show|display|available)\b/i.test(userText);

  let requestedNoteQuery = "";
  const openNoteMatch = userText.match(
    /(?:open|read|show)\s+(?:the\s+)?(?:note\s+)?["“]?([^"”]+?)["”]?\s*$/i,
  );
  if (openNoteMatch?.[1]) {
    requestedNoteQuery = openNoteMatch[1].trim();
  } else if (
    /\bnote\b/i.test(userText) &&
  ...
  const wantsListTasks =
    /\b(list|show|display|all)\b.*\btasks?\b/i.test(userText) ||
    /\btasks?\b.*\b(list|show|display)\b/i.test(userText);

  const addTaskMatch = userText.match(
    /(?:add|create|make)\s+(?:a\s+)?task\s*:?\s*(.+)$/i,
  );
  const taskTitleFromPrompt = addTaskMatch?.[1]?.trim() || "";

  const wantsListSchedule =
    /\b(list|show|display)\b.*\b(schedule|events?|calendar)\b/i.test(
      userText,
    ) ||
...
- If the code needs user input, use sensible default values so it runs standalone.${folderContext}`,
    ...
    if (fenceMatch) codeContent = fenceMatch[1];
  ...
  if (intent.saveToFile) {
    ...
    if (targetDir) {
      ...
      const contentPrompt: AiMessage[] = [
        {
          role: "system",
          content:
            "You are CleverFox, a study assistant. Generate well-structured Markdown content.\nUse proper headings (##, ###), bullet points, and organized sections.\nDo NOT mention saving, Obsidian, or file operations.",
        },
        {
          role: "user",
  ...
  if (actions.length === 0 && !generatedContent && requestedFileFormat) {
    ...
    const fileTopic =
      intent.topic && intent.topic !== userText ? intent.topic : userText;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are Fox Agent, an AI operating system assistant. You have full OS access. Provide a helpful response. Always format your responses elegantly in Markdown.",
          },
          ...userMessages,
        ],
      });
...
export async function executeActions(
  ...
  async function ensureDb() {
    ...
    if (!isMongoConnected()) {
      throw new Error(
        "Database is unavailable right now. Please check MongoDB connection and try again.",
      );
    }
  }
    ...
    try {
      ...
      switch (action.tool) {
        case "create_directory": {
          if (!action.path) throw new Error("create_directory requires path");
        ...
        case "list_directory": {
          if (!action.path) throw new Error("list_directory requires path");
        ...
        case "open_app": {
          if (!action.path) throw new Error("open_app requires path");
        ...
        case "create_task": {
          ...
          if (!title) throw new Error("create_task requires taskTitle");
        ...
        case "schedule_event": {
          ...
          if (!action.eventTitle || !action.eventDate || !action.eventTime) {
            throw new Error(
              "schedule_event requires eventTitle, eventDate, and eventTime (YYYY-MM-DD and HH:MM formats)",
            );
          }
          const startAt = new Date(
            `${action.eventDate}T${action.eventTime}:00`,
          );
        ...
        case "list_schedule_events": {
          ...
          if (events.length === 0) {
            ...
            result = events
              .map(
                (e) =>
                  `- ${e.title} — ${new Date(e.startAt).toLocaleString()}${e.location ? ` @ ${e.location}` : ""}`,
              )
              .join("\n");
          }
          break;
        ...
        case "run_command": {
          ...
          const output = await new Promise<string>((resolve) => {
            let stdout = "";
            let stderr = "";
            r.emitter.on("stdout", (chunk: string) => {
              stdout += chunk;
            });
            r.emitter.on("stderr", (chunk: string) => {
              stderr += chunk;
            });
            r.emitter.on("exit", (info: any) => {
              resolve(
                (stdout || "(no output)") +
                  (stderr ? `\n\nSTDERR:\n${stderr}` : "") +
