...
function recomputeFromSubTasks(task) {
    ...
    task.status = doneCount === 0 ? "pending" : doneCount === total ? "completed" : "inProgress";
...
tasksRouter.post("/tasks", asyncRoute(async (req, res) => {
    ...
    const subTasks = Array.isArray(subTasksRaw)
        ? subTasksRaw
            ...
            .filter(Boolean)
            .slice(0, 100)
            .map((t) => ({ title: t, done: false, createdAt: new Date() }))
        : [];
...
tasksRouter.patch("/tasks/:id", asyncRoute(async (req, res) => {
    ...
    if (subTaskAddRaw !== undefined) {
        ...
        if (!title)
            return res.status(400).json({ ok: false, error: "Invalid subtask title" });
