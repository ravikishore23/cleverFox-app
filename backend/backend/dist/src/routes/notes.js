...
notesRouter.get("/notes", asyncRoute(async (req, res) => {
    await connectMongo();
    ...
    const sortSpec = sort === "createdAt" ? { createdAt: -1 } : { updatedAt: -1 };
...
notesRouter.get("/notes/stats", asyncRoute(async (_req, res) => {
    await connectMongo();
    const notes = await NoteModel.find().exec();
    const total = notes.length;
    const pinned = notes.filter((n) => n.pinned).length;
    const favorites = notes.filter((n) => n.favorite).length;
    const categories = new Set(notes.map((n) => String(n.category ?? "").trim()).filter(Boolean)).size;
...
notesRouter.post("/notes", asyncRoute(async (req, res) => {
    await connectMongo();
...
notesRouter.patch("/notes/:id", asyncRoute(async (req, res) => {
    await connectMongo();
...
notesRouter.delete("/notes/:id", asyncRoute(async (req, res) => {
    await connectMongo();
    const id = String(req.params.id);
