...
const WORKSPACE_ROOT = path.resolve(process.cwd(), "backend", "agent_workspace");
...
export class LocalAgent {
     ...
     */
    resolvePath(filePath, allowAbsolute = false) {
    ...
    async log(message) {
        await appendFile(AGENT_LOG, `${new Date().toISOString()} ${message}\n`).catch(() => { });
    }
    newId() {
        return crypto.randomBytes(12).toString("hex");
    }
    /* ---- File System Tools (used by agentic loop) ---- */
    async listDir(dir = "", allowAbsolute = false) {
    ...
    async readFile(filePath, allowAbsolute = false) {
        const p = this.resolvePath(filePath, allowAbsolute);
        return fs.readFile(p, "utf8");
    }
    async writeFile(filePath, content, allowAbsolute = false) {
        const p = this.resolvePath(filePath, allowAbsolute);
    ...
    async browserSearch(query, browserName = "brave") {
        ...
        exec(`start "" "${browserExe}" "${searchUrl}"`, (err) => {
            if (err)
                this.log(`BROWSER_SEARCH_ERROR: ${err.message}`);
        });
    ...
    async analyzeFolder(dir, maxDepth = 2) {
    ...
    async listRecursiveScan(dir, maxDepth, depth = 0) {
        if (depth >= maxDepth)
            return [];
        ...
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory()) {
                    ...
                    const children = await this.listRecursiveScan(path.join(dir, entry.name), maxDepth, depth + 1);
                    results.push({ name: entry.name, isDirectory: true, children });
                }
                else {
    ...
    buildAnalysisSummary(dir, files, dirs, projectType) {
        let summary = `Scanned "${path.basename(dir)}": ${files} file${files !== 1 ? "s" : ""}, ${dirs} folder${dirs !== 1 ? "s" : ""}`;
        if (projectType)
            summary += ` | Detected: ${projectType} project`;
        if (files === 0 && dirs === 0)
            summary += ` | Empty directory — ready for new files`;
        return summary;
    }
    getEmitter(id) {
