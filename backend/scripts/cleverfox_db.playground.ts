...
async function main() {
  ...
  const PlaygroundItem = mongoose.model(
    "PlaygroundItem",
    new mongoose.Schema(
      {
  ...
  const items = await PlaygroundItem.find()
    ...
    .lean();
  console.log(
    `[playground] Latest items (${items.length}):`,
    items.map((i) => ({ id: String(i._id), name: i.name })),
  );

  await PlaygroundItem.updateOne(
    { _id: created._id },
    { $set: { note: "updated from playground" } },
  );
