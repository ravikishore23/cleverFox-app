...
async function main() {
    ...
    const items = await PlaygroundItem.find().sort({ createdAt: -1 }).limit(5).lean();
    ...
    console.log("[playground] Fetched:", fetched ? { id: String(fetched._id), name: fetched.name, note: fetched.note } : null);
